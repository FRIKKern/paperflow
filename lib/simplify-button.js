 ...
 *   1. POST /simplify {doc_path, goal_id} → {job_id}
 *   2. Poll /simplify/status?job=<id> every 2s, cap 5min
...
(function () {
  ...
  async function startSimplify(docPath, goalId, btn) {
    ...
    let job;
    try {
      const r = await fetch(BRIDGE + '/simplify', {
    ...
    const poll = async () => {
      ...
      let s;
      try {
        const r = await fetch(BRIDGE + '/simplify/status?job=' + encodeURIComponent(job));
        s = await r.json();
      } catch (e) {
