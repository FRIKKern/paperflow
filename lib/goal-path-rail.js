 ...
 * (localhost:8767), not the per-session bridge.
 */
(function () {
  ...
  async function fetchPath() {
    ...
    try {
      ...
      if (r.status === 410) {
        console.warn('[paperflow-rail] daemon returned 410 — proxy not landed?', r.url);
        return [];
      }
      if (!r.ok) return [];
  ...
  async function postActiveBase(eventId) {
    ...
    try {
      const r = await fetch(`${AUX_DAEMON}/paperflow/event/active`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (r.status === 410) {
        console.warn('[paperflow-rail] daemon returned 410 — proxy not landed?', r.url);
        return; // endpoint deferred — no-op
      }
    } catch (_) { /* non-fatal */ }
  }
  ...
  async function simplifyAction(kind) {
    ...
    try {
      ...
      if (r.status === 410) {
        console.warn('[paperflow-rail] daemon returned 410 — proxy not landed?', r.url);
        alert('Simplify is deferred to a follow-up release');
        hideSimplifyToolbar();
        return;
      }
      const j = await r.json();
