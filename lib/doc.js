...
(function () {
  ...
  function ensureMermaidZoom() {
    ...
    css.rel = "stylesheet";
    css.href = "/superpowers/_lib/mermaid-zoom.css";
    document.head.appendChild(css);
    const js = document.createElement("script");
    js.src = "/superpowers/_lib/mermaid-zoom.js";
  ...
  function ensureLiveRender() {
    ...
    const css = document.createElement("link");
    css.rel = "stylesheet";
    css.href = "/superpowers/_lib/live-render.css";
    document.head.appendChild(css);
    const js = document.createElement("script");
    js.src = "/superpowers/_lib/live-render.js";
    document.body.appendChild(js);
  }
      ...
      return [
        {
          ...
          label: "Plan",
          variant: "primary",
          message: `Create an implementation plan from this spec: ${file}. Use the superpowers:writing-plans skill. Write the plan HTML to ~/docs/superpowers/plans/ using the article-style template + the shared /superpowers/_lib/doc.{css,js} for buttons. Use ~/.local/bin/paperflow-target to embed the right CLAUDE_TARGET.`
        },
        {
          id: "grill",
          label: "Grill",
    ...
    if (kind === "note") {
      return [
        ...
        },
        {
          ...
          label: "Promote",
          variant: "secondary",
          message: `Promote this discussion note to a formal spec: ${file}. Read the note in full, distill the core decision/architecture into a new HTML at ~/docs/superpowers/specs/<date>-<topic>-design.html using the spec template.`
        }
      ];
    }
    if (kind === "changelog") {
      ...
      return [
        {
          ...
          label: "Continue",
          variant: "primary",
          message: `Continue this mission: ${file}. First run mission-snapshot to refresh state, then invoke paperflow-continue <slug> via Bash to spawn a fresh Claude session in a new terminal tab.`
        },
        {
          id: "snapshot",
          label: "Snapshot",
          variant: "secondary",
          message: `Snapshot the active mission. Use the mission-snapshot skill to refresh ${file} and its JSON sidecar with current progress, artifacts, and next-step.`
        }
      ];
    }
    return [];
