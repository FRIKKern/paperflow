 ...
 *
 * Buttons POST to <window.CLAUDE_TARGET.bridge_url>/build with a natural-
 * language instruction for Claude in your originating terminal. Claude picks
 * up the right skill. The bridge URL is per-instance — each Claude session
 * runs its own bridge on a unique port, plumbed into CLAUDE_TARGET by
 * ~/.local/bin/paperflow-target. Legacy docs (pre per-instance-bridge
 * migration) fall back to the old hardcoded port and surface as session-gone
 * via the orphan recovery banner.
 */
(function () {
  const STYLE_HREF = "/paperflow/_lib/doc.css";
  const AUX_DAEMON = "http://localhost:8767";

  // Resolve the per-instance bridge base URL ("http://127.0.0.1:NNNN") from
  // window.CLAUDE_TARGET. Returns the legacy fallback when bridge_url is
  // missing — legacy docs surface the orphan banner anyway via the binding
  // status path, so the fallback exists only to prevent a parse error.
  function resolveBridgeBase() {
    const target = window.CLAUDE_TARGET || {};
    if (typeof target.daemon_url === "string" && target.daemon_url) {
      return target.daemon_url.replace(/\/+$/, "");
    }
  ...
  async function fetchBindingStatus() {
    const target = window.CLAUDE_TARGET || {};
    const base = (typeof target.daemon_url === "string" && target.daemon_url)
      || (typeof target.bridge_url === "string" && target.bridge_url);
    if (!base || !target.doc_nonce) {
      // Backwards-compat: legacy docs (pre per-instance-bridge migration)
      // don't carry a daemon_url/bridge_url. Surface as session-gone with a
      // legacy explanation so the recovery banner kicks in.
      return { state: "session-gone", reason: "legacy-binding" };
    }
    try {
      const r = await fetch(`${base.replace(/\/+$/, "")}/docs/${target.doc_nonce}/status`, {
        method: "GET",
      ...
      // Network-level failure (daemon unreachable, ECONNREFUSED in Node /
      // "Failed to fetch" in Chrome) — surface a synthesized 410-Gone-style
      // envelope so the recovery banner appears with a specific reason code.
      if (/Failed to fetch|NetworkError|ECONNREFUSED|fetch failed/i.test(msg)) {
        return { state: "session-gone", reason: "daemon-unreachable", code: "ECONNREFUSED" };
      }
      return { state: "session-gone", reason: msg };
    }
  }
  ...
  async function applyStatus(status) {
    const prevState = bindingState.state;
    const stateChanged = prevState !== status.state;
    bindingState.state = status.state;
    bindingState.last = status;

    if (bindingState.pill && stateChanged) {
      ...
      // aria-label, not title — native title tooltips hide/re-show on every
      // mousemove inside the element, which reads as a flicker. The pill's
      // visible color (via [data-state] CSS) is the state indicator; aria-label
      // is there only for screen readers.
      bindingState.pill.setAttribute("aria-label", ts);
    }
    ...
    // Orphan branch — only rebuild banner DOM when the state actually
    // changed. Repeated polls in the same orphan state would otherwise
    // tear down and reappend the banner on each call (visible flicker).
    if (!stateChanged && bindingState.banner) return;
      ...
      return [
        {
          ...
          label: "Plan",
          variant: "primary",
          message: `Create an implementation plan from this spec: ${file}. Use the /paperflow:plan skill. Write the plan HTML to ~/docs/paperflow/plans/ using the article-style template + the shared /paperflow/_lib/doc.{css,js} for buttons. Use \`~/.local/bin/paperflow-target <plan-html-path>\` (path argument required) to embed the right CLAUDE_TARGET — the helper registers the doc_nonce with the bridge as a side-effect.`
        },
        {
          id: "grill",
          label: "Grill",
  ...
  async function dispatch(action, message, btn, status, allBtns) {
    ...
    try {
      ...
      if (/Failed to fetch|NetworkError|ECONNREFUSED|fetch failed/i.test(msg)) {
        status.textContent = "daemon unreachable (ECONNREFUSED)";
      } else {
  ...
  function init() {
    ...
    pill.setAttribute("aria-label", "Bridge status — checking…");
    ...
    // Expose refreshBindingStatus on window.paperflow so cross-script callers
    // (notably grill.js's marker + build POSTs) can request an immediate status
    // refresh after a failed POST without having to wait for visibilitychange.
    window.paperflow = window.paperflow || {};
    window.paperflow.refreshBindingStatus = refreshBindingStatus;
    ...
    // Trigger #4 — cross-script failure signals. grill.js (and any other
    // future cooperating script) dispatches these events when its own POST
    // to /marker or /build fails with stale-binding / session-gone. We
    // refresh immediately so the pill + recovery banner update without the
    // user needing to refocus the tab.
    window.addEventListener("paperflow:marker-failed", () => refreshBindingStatus());
    window.addEventListener("paperflow:build-failed", () => refreshBindingStatus());
