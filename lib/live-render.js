...
(function () {
  ...
  // ── 0. Off-localhost short-circuit ─────────────────────────────────
  // Printed PDFs, emailed copies, USB exports, file:// opens, and any
  // off-localhost browser shouldn't try to open a WebSocket back to a
  // dev server that doesn't exist. Bail out before any other work.
  // See spec v7 § "Live-render" — figure 8.
  const ALLOW_HOSTS = ["localhost", "127.0.0.1", "::1"];
  if (!ALLOW_HOSTS.includes(location.hostname)) {
    if (window.console && console.debug) {
      console.debug("[paperflow live-render] disabled — host",
                    location.hostname, "not in allowlist");
    }
    return;
  }
  ...
  // ── 5b. Zero-layout-shift insert (P9) ──────────────────────────────
  // When we insert a brand-new node, allocate height of 0, then on the
  // next frame transition to the node's natural scrollHeight + opacity 1.
  // Content below slides in lockstep with the fade — no jump.
  //
  // After the transition ends, we clear the inline max-height/overflow so
  // the node is fully responsive (no clipping, no cap if it later grows).
  //
  // For pre.mermaid we defer the animation trigger until Mermaid has
  // rendered the SVG — otherwise scrollHeight is 0 and the animation is
  // invisible. A MutationObserver watches for an <svg> child to appear.
  ...
  // Stage a freshly-inserted element for the grow animation. We mark it
  // .lr-grow and set the collapsed inline styles. The actual transition
  // is triggered later by triggerGrowAnimation() — either right after
  // morph for normal nodes, or after Mermaid render for pre.mermaid.
  function stageForGrow(el) {
    if (!el || el.nodeType !== 1) return;
    if (prefersReducedMotion()) {
      // Just opacity, no height clamp — keep it cheap and instant-ish.
  ...
  // Drive the grow transition on a staged element. For pre.mermaid we
  // wait for the rendered <svg> child before measuring — otherwise the
  // collapsed height never opens, since the empty source has height 0.
  function triggerGrowAnimation(el) {
    if (!el || el.nodeType !== 1) return;
    if (!el.classList.contains("lr-grow")) return;

    // Find an internal pre.mermaid that hasn't rendered yet.
    const innerMermaid = el.matches && el.matches("pre.mermaid")
      ? (isRenderedMermaidEl(el) ? null : el)
      : (el.querySelector ? el.querySelector("pre.mermaid:not([data-processed='true'])") : null);

    if (innerMermaid && !innerMermaid.querySelector("svg")) {
      // Defer until Mermaid completes — observe for an <svg> child.
      const target = innerMermaid;
      const obs = new MutationObserver(() => {
        if (target.querySelector("svg")) {
          obs.disconnect();
          // Next frame so layout sees the svg before we measure.
          requestAnimationFrame(() => runGrow(el));
        }
      });
      obs.observe(target, { childList: true, subtree: true });
      // Safety fallback: if Mermaid never fires (or the svg never
      // arrives), just open the node after 2s using whatever height
      // it has.
      setTimeout(() => {
        try { obs.disconnect(); } catch (_) {}
        if (el.classList.contains("lr-grow")) runGrow(el);
      }, 2000);
      return;
    }

    // Two rAFs: first to settle the collapsed style, second to start
    // the transition. Single-rAF can race with style flush in Chromium.
    requestAnimationFrame(() => requestAnimationFrame(() => runGrow(el)));
  }
  ...
  function runGrow(el) {
    ...
    const clear = () => {
      ...
      el.classList.remove("lr-grow");
      // Drop the inline clamp so the node is freely responsive again.
      el.style.maxHeight = "";
      el.style.overflow = "";
      el.style.transition = "";
      el.style.opacity = "";
    ...
    const onEnd = (ev) => {
      // Multiple transitions fire (max-height + opacity). Wait for the
      // last one — but a single fire is enough to detect we've started.
  ...
  async function runMorph() {
    ...
    try {
      ...
      // Animate any nodes pre-staged with .lr-grow into their natural height.
      // Mermaid containers wait for their SVG before triggering (see helper).
      document.querySelectorAll(".lr-grow").forEach(triggerGrowAnimation);
  ...
  function morphBody(liveBody, newBody) {
    ...
    for (let i = 0; i < newChildren.length; i++) {
      ...
      if (match && !isPreserved(match)) {
        ...
        // Pre-stage zero-height + transparent so the insert doesn't kick
        // content below downward in one jump (P9). Animation trigger is
        // dispatched after the morph sweep completes.
        if (fresh.nodeType === 1) stageForGrow(fresh);
