 ...
 * structured answers back through the claude-bridge.
 *
 * The same renderer powers two artifact types:
 *   - kind: "grill"         (default; mid-plan adversarial pressure)
 *   - kind: "questionnaire" (upfront discovery before plan draft)
 * Only the bridge-message header label and the optional `Goal:` line
 * differ — submit-button text and form ergonomics are identical.
 *
 * Question schema:
...
(function () {
  ...
  // For now: include a structured field in the POST body that paperflow's
  // bridge can pick up; if the bridge doesn't yet honour it, the marker
  // is harmless. paperflow-resume's scan is the ultimate consumer.
  async function writeAnsweredMarker(grill) {
    if (grill.kind !== "questionnaire") return;
    if (!grill.plan) return;
    try {
      await fetch(BRIDGE.replace(/\/build$/, "/marker"), {
        method: "POST",
      ...
      // Marker is best-effort. Failure here doesn't break the submit
      // flow — paperflow-resume falls back to "no sidecar = unfinished".
    }
  }
