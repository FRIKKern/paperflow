---
name: mission-create
description: Use when the user says "start a mission", "new mission for X", "let's begin a project to do Y", or kicks off any non-trivial multi-artifact piece of work that will span multiple specs / plans / grills. Picks a kebab-case slug, spawns a subagent to write a paired HTML + JSON mission file under ~/docs/superpowers/missions/, and marks it active in ~/.paperflow/active-mission so mission-snapshot and mission-continue know which mission to operate on.
---
...
2. **Spawn a subagent (`subagent_type: general-purpose`)** to write both files in parallel:
   - `~/docs/superpowers/missions/<date>-<slug>.html` — article-style HTML mirroring the spec template.
   - `~/docs/superpowers/missions/<date>-<slug>.json` — machine-readable sidecar.
...
4. **Reply** with the localhost URL: `http://localhost:8765/superpowers/missions/<date>-<slug>.html`.
...
```json
{
  ...
  "artifacts": [
    { "type": "spec", "path": "/superpowers/specs/2026-05-02-...html", "title": "..." }
  ],
  "decisions": [
  ...
  "open_questions": ["..."],
  "resume_prompt": "You are continuing the <name> mission. Read /Users/frikkjarl/docs/superpowers/missions/<slug>.html in full. Then do: <next-step>."
}
```
...
- **H2 Artifacts** — table or bullet list of every artifact (link via `/superpowers/<type>s/<file>`). Empty section if none yet.
...
- **Tail** — the standard `window.CLAUDE_TARGET` + `window.DOC_PATH` + `<script src="/superpowers/_lib/doc.js">` so the mission HTML gets Continue / Snapshot buttons.
