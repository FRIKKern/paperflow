...
3. **Explain what will happen.** Print a plain-language summary of what `install.sh` does (host-scoped paperflow-daemon on :8767, dock daemon at ~/.paperflow/dock.sock, statusline, ~/.claude/CLAUDE.md write, ~/.local/bin/ symlinks, optional brew installs).
